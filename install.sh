...
setuponly() {
groupadd gameservers;echo "DenyGroups gameservers" > /etc/ssh/sshd_config;rpm --import /etc/pki/rpm-gpg/RPM-GPG-KEY*;yum install epel-release -y;yum install htop -y;yum install fail2ban -y;yum install iftop -y;yum install pure-ftpd;systemctl enable pure-ftpd;systemctl start pure-ftpd;yum install java-1.8.0-openjdk -y;yum install java-1.8.0-openjdk;yum install glibc.i686 libstdc++.i686 -y;yum install screen -y;
echo "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
